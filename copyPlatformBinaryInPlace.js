...
var filename = 'bin/lang_ppx-' + platform + '-' + arch + '.exe';
var supported = fs.existsSync(filename);

if (!supported) {
    console.error('lang_ppx does not support this platform :(');
    console.error('');
    console.error('lang_ppx comes prepacked as built binaries to avoid large');
    console.error('dependencies at build-time.');
    console.error('');
    console.error('If you want lang_ppx to support this platform natively,');
    ...
    if (!process.env.IS_lang_PPX_CI) {
        process.exit(1);
    }
}

if (process.env.IS_lang_PPX_CI) {
    console.log('lang_ppx: IS_lang_PPX_CI has been set, skipping moving binary in place');
    process.exit(0);
}
