...
module Person = {
  class t (name: string, surName: string) = {
    inherit class Lang.Any.t as super;
    pub personName = name ++ " => " ++ UUID.makeV4();
  };

  module ClassType = {
    type Lang.Any.ClassType.variant('a) +=
      | AnyClass(t);

    type Lang.Any.ClassType.variant('a) +=
      | PersonClass(t);

    type variant('a) = Lang.Any.ClassType.variant('a) = ..;
  };
};

/* let person = (new person)("wilton", "lazary");

   print_endline(person#personName);
   print_endline(person#personName); */